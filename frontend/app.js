...
async function queryQuestion(ev) {
  ...
  const topK = parseInt($id('topK').value || '5', 10);
  if (!question) return;
  ...
  if (isPreview()) {
    setTimeout(() => {
      ...
      chatHistory.push({
        role: 'ai',
        content: `This is a preview answer to: "${question}".\n(Preview responses are synthetic and for UI testing only.)`,
        sources: ['Preview_Doc_1.txt', 'Preview_Doc_2.txt'],
        context: [
          { id: 'Preview_Doc_1_0', content: 'Sample context chunk 1 about decisions and action items...', source: 'Preview_Doc_1.txt', chunk_id: 0, score: 0.98 },
          { id: 'Preview_Doc_2_1', content: 'Sample context chunk 2 with attendee list and decisions...', source: 'Preview_Doc_2.txt', chunk_id: 1, score: 0.87 }
        ]
      });
      renderChat();
  ...
  try {
    ...
    chatHistory.pop();
    if (r.ok) {
      chatHistory.push({ role: 'ai', content: json.answer, sources: json.sources, context: json.context });
    } else {
      chatHistory.push({ role: 'ai', content: `Error (${r.status}): ${JSON.stringify(json)}` });
    }
    renderChat();
  } catch (err) {
    chatHistory.pop();
